					...
					@if($article->parent_id==0) <?php $parent_article_id = $article->id ?>
						...
						<tr data-id="{{$article->id}}" data-url="{{route($module_info['module'].'.update', $article->id)}}">
	                      ...
	                      <td class="align-middle full-width pr-3"><a href="{{route($module_info['module'].'.edit', $article->id)}}">
							  ...
							  @else
								<span style="color:red">Нет названия на языке [{{ $LANG }}]</span>
							  @endif
							  </a>
								@if($article->sub == 'yes') &nbsp; <a href="{{route($module_info['module'].'.create')}}?parent_id={{$article->id}}" title="Создать дочернюю страницу"><span><i class="fas fa-plus"></i></span></a>@endif
							  ...
							  @else
								<td class="align-middle" title="Позиция в списке"><span class="iposition"><i class="fas fa-arrows-alt"></i> {{ $article->position }}</span></td>
							  @endif
	                      ...
	                      <td class="align-middle text-right">
	                      	<div class="btn-group">
		                        ...
		                        @endif
								<a href="{{route($module_info['module'].'.edit', $article->id)}}" class="btn btn-sm btn-flat" title="Редактировать"><i class="far fa-edit"></i></a>
