...
@section('content')
<form action="{{ (request()->routeIs('users-aml.edit')) ? route('users-aml.update', $manager->id): ''}}{{ request()->routeIs('users-aml.create') ? route('users-aml.store'): ''}}" method="post"  enctype="multipart/form-data" id="form">@csrf
@if (request()->routeIs('users-aml.edit'))
	@method('PUT')
@endif
  ...
  <div class="content-wrapper form_zone">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-8">
            <h1 class="m-0 text-dark">Участники МСИ <span>/ {{ Config::get('cms.modules.uchastniki-aml.sub.users-aml.name') }}</span> <span>- {{ request()->routeIs('users-aml.edit') ? 'Редактирование': ''}} {{ request()->routeIs('users-aml.create') ? 'Создание' : ''}}</span></h1>
          </div><!-- /.col -->
          <div class="col-sm-4">
            <button type="submit" class="btn btn-info float-right ml-2 mb-2 trigger_btn_save">Сохранить</button>            
			<a href="{{route('users-aml.index')}}" class="btn btn-primary float-right">Назад</a>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
                    ...
                    <div class="col-sm-12">
                      ...
                      <div class="form-group">
                        <div class="custom-control custom-checkbox">
                          <input class="custom-control-input" type="checkbox" id="active" name="active" value="1" @if ($manager->active || request()->routeIs('users-aml.create')) checked @endif>
					 ...
					 @endif
					  <div class="form-group">
						<label>Организация МСИ *</label>
						<select class="form-control select2" required name="aml_artile_id">
						<option value="0">-- Не указано --</option>
						@foreach ($organizations as $r)							
							<option value="{{$r->id}}" @if(!empty($r->uchastniki_aml_connect_user->id) && $manager->aml_artile_id != $r->id) disabled title="К этой организации уже привязан другой пользователь: {{$r->uchastniki_aml_connect_user->surname}} {{$r->uchastniki_aml_connect_user->name}} {{$r->uchastniki_aml_connect_user->patronymic}}" @endif @if($manager->aml_artile_id == $r->id) selected @endif>{{$r->details_one->name}}</option>							
						@endforeach
						</select>
                     </div>
                    </div>
