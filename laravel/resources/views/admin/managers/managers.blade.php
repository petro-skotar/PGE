            ...
            <div class="card">
			  ...
			  <div class="card-body table-responsive p-0">
                <table class="table table-sm table-hover text-nowrap listing ui-sortable">
                  @foreach($managers as $manager)
	                    <tr data-id="{{$manager->id}}" data-url="{{route('managers.update', $manager->id)}}" >
	                     	<td class="align-middle">
	                      		<span class="handle">
		                      		<i class="far fa-user"></i>
                     			</span>
                     		</td>
	                      <td class="align-middle"><a href="{{route('managers.edit', $manager->id)}}">{{$manager->name}}</a></td>
	                      <td class="align-middle pl-4 pr-4">{{$manager->email}}</td>
	                      <td class="align-middle full-width">{{$manager->roles_one($manager->role_id)['name']}}</td>
						  <td class="align-middle">
							@if ($manager->id != 1)
			  ...
			  @else
				<div class="card-body table-responsive">
					...
					@if($table_search)
						<p><a href="{{ route('managers.index') }}">Назад к списку</a></p>
					@endif
				</div>
            @endif
              <!-- /.card-body -->
